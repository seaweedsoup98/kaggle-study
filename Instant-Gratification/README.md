...
│   └── 06_label_noise_ceiling.ipynb    # 라벨 노이즈 진단과 성능 천장 — denoise/calibrate 산출물 분석
└── experiments/                   # 실험마다 폴더 하나, 결과물은 각자 outputs/ 에
    ...
    │   ├── pseudo_labeling.py
    │   └── outputs/
    ├── simplify/                  # 추가 실험 3 — baseline 해부 & 단순화 (7.3절)
    │   ├── simplify.py
    │   └── outputs/
    ├── denoise/                   # 추가 실험 4 — 라벨 노이즈 진단·제거 (7.4절)
    │   ├── diagnose.py            #   Step 0: 노이즈 존재·비율·탐지 가능성 (재학습 없음)
...
```bash
# 9) 라벨 노이즈 — 진단(40s, 8 의 OOF 필요) → 제거 학습(~30분) → 합성 검증(30s). eda/06 이 이 산출물을 읽는다
python experiments/denoise/diagnose.py
python experiments/denoise/denoise.py
python experiments/denoise/denoise_synthetic.py
```
...
읽는 순서는 01 → 02 → 03 → 04 → 05 → 06 이며, 각 노트북 끝에 `직접 해보기` 연습문제가 있다.
04·05·06 은 실험 산출물(`experiments/*/outputs/`)을 읽으므로 해당 스크립트를 먼저 완주해야 한다.
...
### 4.6 [`06_label_noise_ceiling.ipynb`](eda/06_label_noise_ceiling.ipynb) — 라벨 노이즈와 천장

"왜 모든 방법이 0.95 에서 멈추는가"의 답. `experiments/denoise/`, `experiments/calibrate/` 산출물을 읽는다(몇 초).

| 절 | 내용 |
|---|---|
| 노이즈 탐지 3종 | 확신 오답의 임계값 둔감성(4.5%→5.1%) · 혼합비 추정 f=a/(a+b) (0.0501~0.0504) · 그룹 간 이항 균일성(sd 비 0.94) |
| 천장 공식 | 라벨의 f 가 뒤집혀 있으면 완벽한 분류기도 AUC ≤ **1 − f = 0.95**. 우리 점수 전부가 그 0.0004 안 |
| 합성 대조 | 같은 모델이 뒤집힌 라벨로는 0.9498, 진짜 라벨로는 **0.9995** — 격차는 전부 라벨 노이즈 |
| 노이즈 제거 학습 | 그룹 안 순위는 +0.0015 유의하게 좋아지지만 채점 점수는 +0.0001 — 천장 |
| 캘리브레이션 착시 | in-sample +0.0022 vs 교차 적합 −0.0006 — 차이 전부가 flip 실현에 대한 과적합 |
| 수수께끼 해소 | 02 의 "측정 오차 83%" = 그룹마다 걸린 flip 수(Spearman −0.845). 04 의 "잡음 추종" = flip 이 많이 걸린 그룹 |
...
### 7.3 해부 & 단순화 — [`experiments/simplify/`](experiments/simplify/)

"baseline 이 너무 복잡하다"는 의심에서 출발해 두 질문에 답했다.
**① 부품 중 무엇이 성능을 만드는가** **② 결정적인 부품만 남기면 같은 점수가 나오는가.**

설계: 프로브 모델은 **QDA 하나**(baseline 6개 중 최고·최경량). **512그룹 전체**, 같은 fold,
그룹별 대응 비교(95% CI). 시작 설정(`baseline_probe`)이 baseline 의 (kmeans, seed 1) QDA 를
**비트 단위로 재현**하는지 검사했다 — `0.9490644878826664`, |diff| = 0.
설정 40개, 총 54분 + 18분.

#### ① 해부 — 부품을 하나씩 빼면 (baseline 피처 + QDA 기준, 그룹별 대응 Δ)

| 뺀 부품 | pooled AUC | Δ vs 원본 (95% CI) | 이긴 그룹 | 판정 |
|---|---|---|---|---|
| (원본 그대로) | 0.949064 | — | — | |
| **transductive 적합** | 0.930463 | **−0.0157** [−0.0165, −0.0148] | 20/512 | 결정적 |
| **GMM 소속확률** | 0.935036 | **−0.0111** [−0.0119, −0.0103] | 55/512 | 결정적 |
| **StandardScaler** | 0.944537 | **−0.0078** [−0.0085, −0.0071] | 54/512 | 결정적 |
| KernelPCA | 0.948687 | −0.0003 [−0.0009, +0.0003] | 255/512 | 그룹별 무효 — 단 pooled 는 −0.0004 (아래 참고) |
| GMM 로그밀도 ×3 | 0.949049 | −0.0001 [−0.0001, +0.0000] | 243/512 | 무효 |
| hist 밀도 피처 | 0.949109 | +0.0000 [−0.0000, +0.0001] | 244/512 | 무효 |
| **GMM 라벨 stratify** | 0.949547 | **+0.0005** [+0.0002, +0.0008] | 285/512 | **빼면 좋아짐** |
| **소속확률 ×5 반복복사** | 0.949143 | **+0.0007** [+0.0005, +0.0009] | 309/512 | **빼면 좋아짐** |

부품 9개 중 **3개가 성능을 만들고, 2개는 해롭고, 4개는 장식**이다.

- GMM 을 빼고 대신 쓸 만한 것은 없었다. 원시 피처 QDA 0.9366, 클래스별 GMM 생성분류기(k=1/2/3)
  0.9365 / 0.9357 / 0.9290, GMM 공분산을 diag 로 바꾸면 0.9323. **train+test 를 합쳐 적합한
  full-covariance GMM 의 소속확률**이 이 문제의 메커니즘 자체다.
- GMM 군집 수를 512그룹에서 다시 스윕: k=3 0.9437 · 4 0.9470 · **5 0.9487** · 6 0.9485 · 7 0.9479. k=5 확정.
- QDA `reg_param` 0.111 → 0.3 → 0.5: 그룹별로는 +0.0005 → +0.0014 → +0.0016 으로 유의하게 오르지만
  pooled 는 0.94875 → 0.94878 → 0.94866 으로 평평하다. 정규화를 키우면 그룹 안 순위는 좋아지고
  그룹 간 스케일은 나빠진다.

#### ② 단순화 — 결정적인 부품만 남기면

`lean` = **std>2 선택 → KernelPCA(cosine, 차원 유지) → GMM(5, full) 소속확률 1회 → StandardScaler → QDA**,
target 으로 stratify 한 5-fold, train+test 합쳐 적합. 이걸 GMM 시드만 바꿔 평균한다.

| 파이프라인 | pooled AUC | Δ vs baseline 최종 (95% CI) | 그룹당 fit 수 | 실행 시간 |
|---|---|---|---|---|
| **baseline** (6모델 × 4config + LGBM·MLP 메타) | 0.949605 | — | ~136 + 메타 | 2,617s |
| baseline 의 QDA 만 4config 평균 | 0.949259 | | | |
| lean, 시드 1개 | 0.949546 | −0.0006 [−0.0012, −0.0001] | 8 | 127s |
| lean, 시드 2개 평균 | 0.949684 | −0.0001 [−0.0006, +0.0004] | 16 | 254s |
| **lean, 시드 4개 평균** | **0.949699** | **+0.0005 [−0.0000, +0.0011]** | **32** | **509s** |
| baseline 피처 + target stratify, 시드 4개 | 0.949763 | +0.0000 [−0.0005, +0.0006] | 36 | 513s |
| minimal(KernelPCA 없음) 12개 평균 | 0.949368 | +0.0014 [+0.0008, +0.0020] | 84 | 710s |

**결론: `lean` 시드 4개 평균이 baseline 최종과 통계적으로 구분되지 않는 점수를
1/5 의 시간, 1/4 의 학습 횟수, 모델 1종·스태킹 없음으로 낸다.**
pooled 는 +0.0001 높지만 이 크기는 잡음이고(7.1의 판정 기준), 그룹별 CI 도 0 을 포함한다.
"더 좋게"는 입증되지 않았고 **"같은 성능을 훨씬 가볍게"** 는 입증되었다.

주의 두 가지:
- **시드 하나로는 안 된다.** 같은 설정의 시드별 pooled 가 0.9485 ~ 0.9495 로 ±0.0005 흔들린다.
  시드 1개짜리 결과가 baseline 과 같아 보이는 것은 운이다. 최소 2개, 권장 4개.
- **KernelPCA 는 남겨야 한다 — 이유가 의외다.** 그룹별 AUC 로는 무효인데, 뺀 `minimal` 계열은
  그룹별로 baseline 을 유의하게 이기면서(+0.0014, 289/512) **pooled 로는 −0.0002 진다.**
  [04](eda/04_model_diagnostics.ipynb) 3절이 찾은 "그룹 간 확률 스케일" 현상과 같은 것이다 —
  KernelPCA 의 실제 역할은 그룹 안 순위가 아니라 **512개 그룹의 확률 스케일을 서로 비교 가능하게 맞추는 것**이다.

산출물: `outputs/simplify_metrics.json`(1차 32개 설정), `outputs/followup_metrics.json`(후속 8개 + 조합),
`outputs/*_per_group_auc.csv`, 가장 가벼운 합격 후보(`lean` 시드 2개)의 `outputs/followup_submission.csv`.

### 7.4 라벨 노이즈 — 진단, 제거 학습, 그리고 천장 — [`experiments/denoise/`](experiments/denoise/)

스터디원의 질문에서 출발했다: *"합성 데이터니까 일부러 노이즈를 넣었을 텐데(대략 5%?), 빼고 학습하면
더 단순한 모델로 좋은 성능이 나오지 않을까?"* `make_classification` 의 `flip_y` 는 라벨 일부를 무작위로 뒤집는다.
학습 노트북은 [06](eda/06_label_noise_ceiling.ipynb).

#### Step 0 — 진단 (`diagnose.py`, 재학습 없음, 40초)

| 측정 | 결과 | 해석 |
|---|---|---|
| 확신 오답 비율, τ 0.99 → 0.6 | 4.52% → 5.12%, 두 클래스 대칭(0.0452 / 0.0453) | τ 에 둔감 = 모델 오류가 아니라 **라벨 flip** |
| 뒤집힘 비율 추정 f = a/(a+b) | **0.0501 ~ 0.0504** (t 5개 × 양쪽 꼬리 10개 값 전부) | **라벨의 5.0% 가 뒤집혀 있다** |
| 그룹별 균일성 | 관측 sd / 이항 sd = 0.94, 그룹 AUC 와 Spearman **−0.845** | 그룹 간 균일한 무작위 flip. **02 의 "측정 오차 83%" 의 정체 = 그룹마다 걸린 flip 수** |
| 합성 대조 (flip 0.05) | AUC(뒤집힌 라벨) **0.9498** / AUC(진짜 라벨) **0.9995** | 실제 0.9497 과 일치. **모델은 거의 완벽, 격차는 전부 라벨 노이즈** |

**천장**: 라벨의 f 가 뒤집혀 있으면 완벽한 분류기도 AUC 는 (1−f)² + 2f(1−f)·½ = **1 − f = 0.95** 가 최대다.
baseline 0.949605, lean 0.949699, 가중 앙상블 최고 0.949943, pseudo labeling 0.949614 — **전부 천장에서 0.0004 안**에 있다.
7.1~7.3 의 실험이 모두 "차이 없음"으로 끝난 이유가 여기 있다: 방법이 나빠서가 아니라 채점 라벨의 5% 를 맞힐 방법이 없어서다.

#### Step 1 — 제거 학습 (`denoise.py`, 512그룹 · 4시드, 24분)

lean 의 outer fold 안에서 **학습 fold 만으로 내부 5-fold** 를 돌려 확신 있게 라벨과 어긋나는 행을 제거(또는 뒤집기)한 뒤 재적합.
검증 fold 의 라벨은 채점에만 쓴다. 대조군(τ=1.0)이 lean 을 **비트 단위로 재현**했다(|diff| = 0).
...
| drop τ=0.99 | 0.949825 | +0.00013 | +0.00140 | 354/512 | 5.1% |
| flip τ=0.9 | 0.949852 | +0.00015 | +0.00146 | 342/512 | 5.6% |
| drop τ=0.9, 2회 반복 | 0.949822 | +0.00012 | +0.00154 | 349/512 | 5.9% |
| drop τ=0.9, reg 0.05 | 0.949851 | +0.00015 | +0.00145 | 352/512 | 5.7% |
| raw QDA (GMM 피처 없음) | 0.939301 | −0.0104 | −0.00916 | 48/512 | 0 |
| raw QDA + drop τ=0.9 | 0.940973 | −0.0087 | −0.00708 | 86/512 | 7.0% |

합성 그룹(진짜 라벨로 채점, `denoise_synthetic.py`): lean **0.9992 → 0.9992** (변화 없음), raw QDA 0.9861 → 0.9884 (+0.0023).

- **그룹 안 순위는 실제로 좋아진다** — 그룹별 AUC +0.0015 유의, 354/512 승. 그룹 평균 0.9479 → 0.9494, 천장까지 0.0006.
- **채점 점수(pooled)로는 안 보인다** — +0.00012~0.00015, 잡음 기준과 같은 크기. 천장까지 남아 있던 0.0003 의 절반.
- **제거된 비율 5.1~5.6% = 추정 f.** 탐지기가 노이즈만큼만 뺀다 (합성에서 정밀도 0.92, 재현율 0.98).
- **lean 은 이미 노이즈에 강하다** — 합성 진짜 라벨 AUC 가 제거 전후 그대로. GMM 은 라벨을 안 쓰고, QDA 의 `reg_param` 이 오염을 덮고 있었다.
- **단순한 모델은 따라잡지 못한다** — raw QDA 는 노이즈를 빼도 lean 과 0.009 격차. 격차의 원인은 노이즈가 아니라 **GMM 피처(transductive)의 부재**.
- drop/flip, 1회/2회, reg 0.111/0.05 는 서로 구분되지 않는다.

**판정**: 질문의 전반("노이즈 5%")은 정확히 맞았고, 후반("빼면 더 좋아진다 / 단순 모델이 따라잡는다")은 기각됐다.
**lean 에는 넣지 않는다** — 복잡도가 늘고 채점 점수 이득이 없다. 하지만 이 가설을 검증하는 과정에서
**"왜 모든 방법이 0.95 에서 멈추는가"** 의 답이 나왔다. 나머지 실험 전부를 합친 것보다 값진 결과다.
...
(`reg_param` 을 키우면 그룹별 +0.0016 인데 pooled 는 평평, `minimal` 은 그룹별로 이기고 pooled 로는 짐,
`logreg_l1` 은 그룹 내 꼴찌인데 pooled 2위). "그룹마다 QDA 확률 스케일이 달라서"라는 가설을
그룹별 보정기(temperature / Platt / bias, 교차 적합)로 검증했다. 재학습 없이 lean OOF 만 읽는다(1분).

| 방법 | pooled AUC | Δ vs lean | 그룹 부트스트랩 95% CI |
|---|---|---|---|
| temperature (교차 적합) | 0.949062 | **−0.00064** | [−0.00102, −0.00029] |
| platt / bias (교차 적합) | 0.948756 / 0.948791 | −0.00094 / −0.00091 | 유의하게 손해 |
| rank / zscore (라벨 불필요) | 0.948404 / 0.948853 | −0.00130 / −0.00085 | 유의하게 손해 |
| platt **in-sample** | 0.951933 | +0.00223 | 아래 참고 |
| clip p∈[0.01, 0.99] | 0.949832 | +0.00013 | 잡음 수준 |

- **교차 적합한 모든 보정이 손해다.** 그룹당 512행으로 맞춘 보정기의 잡음이 회수하려는 이득보다 크다.
- **in-sample +0.0022 는 상한이 아니라 과적합의 크기다.** 라벨의 5% 가 무작위로 뒤집혀 있으므로(7.4절)
  보정기가 그 그룹에 실현된 flip 개수에 맞춰지면 pooled 가 오른 것처럼 보인다. 교차 적합에서 부호가 뒤집힌다.
- **클리핑 +0.00013 은 flip 가설과 방향이 맞는다** — 뒤집힌 라벨의 행은 다른 클래스 군집 한가운데 있어
  가장 극단적인 확률을 받으므로, 극단값들의 순서를 동점으로 없애면 손해가 준다(그룹별 AUC +0.0019).
  pooled 로는 잡음 수준이라 채택하지 않는다.
...
**해볼 가치가 있는 것**
- **리더보드로 천장 확인** — 7.4절의 결론은 "채점 라벨의 5% 가 뒤집혀 있어 pooled AUC 의 천장이 0.95" 다.
  이 추정은 train 라벨로만 한 것이므로, `experiments/lean/outputs/lean_submission.csv` 를 제출해
  public LB 가 0.95 근처인지 확인하는 것이 마지막 검증이다. test 의 flip 비율이 다르면 그 차이가 LB 에 나타난다
- **다른 대회로** — 이 데이터셋에서 남은 개선 여지는 pooled 0.0003 이다. 같은 방법론(EDA 노트북 시리즈, 실험 폴더 규약,
  비트 재현 검사, 대응 비교)을 다음 대회에 옮기는 것이 더 큰 수확이다

**시도했지만 천장 논리로 닫은 것**
- ~~**모델 다양성 확보**~~ — level-1 상관 0.89~0.99 는 사실이지만, 새 모델이 얻을 수 있는 최대치가 0.0003 이라 비용 대비 의미가 없다
- ~~**그룹 간 확률 캘리브레이션**~~ — 실행했다(7.5절). 교차 적합하면 전부 손해, in-sample 이득은 flip 과적합.
  pooled/그룹별 분기의 원인은 그룹 스케일이 아니라 **그룹마다 걸린 flip 수**였다
...
> 다섯 갈래(가중 앙상블 · pseudo labeling · 하이퍼파라미터 · 노이즈 제거 · 캘리브레이션)가 모두 "차이 없음"으로
> 끝난 이유는 하나다: **채점 라벨의 5.0% 가 뒤집혀 있어 pooled AUC 의 천장이 1 − f = 0.95 이고,
> baseline 부터 lean 까지 전부 그 천장에서 0.0004 안에 있다** (7.4절). 합성 데이터에서 같은 모델은 진짜 라벨 기준 0.9995 다.
> 이 저장소에서 배울 것은 "어떻게 0.95 를 넘는가"가 아니라 **"최적화를 시작하기 전에 천장을 먼저 계산하라"** 였다 —
> 40초짜리 진단([06](eda/06_label_noise_ceiling.ipynb))이 수 시간의 실험보다 먼저였어야 했다.
